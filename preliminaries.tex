...
\note{todo este párrafo debería explicarse mejor una vez que esté hecha la parte del lema topológico}
...
\note{acá o bien en la parte del lema topológico habría que probar algunas propiedades básicas de la topología -- cuál es la noción de convergencia, etc.}
...
\]
that is, the $R$-submodule spanned by cycles of length $n$. A \emph{potential} $S$ is an element of the closed \note{por qué $\KQ_{\cyc}$ es cerrado?} $R$-submodule $\KQ_{\cyc}\subseteq \KQ$, which we define as
\[
...
\begin{prop}[{\cite{FST08}*{Proposition 2.10}}] Any ideal triangulation of a surface with marked points $(\Sigma, M)$ consists of exactly
\[
n=6g+3b+3p+c-6
\]
...
\begin{prop}[{\cite{FST08}*{Proposition 2.13}}] Let $(\Sigma, M)$ be any surface with marked points not on the list above. Then $(\Sigma, M)$ admits an ideal triangulation involving no self-folded triangles. \hfill\qedsymbol
\end{prop}
\end{section}
\end{chapter}