...
{\begin{beamercolorbox}{section in head/foot}
\vskip2pt\insertshortauthor[width={12cm},center]
\hskip2pt\insertframenumber\vskip2pt
\end{beamercolorbox}}
...
\medskip
\pause
\[y^2x =y(yx) \pause  \rightsquigarrow y(xy+1) = (yx)y + y \pause \rightsquigarrow (xy+1)y + y = xy^2 + 2y\]
\end{frame}
...
\begin{frame}
\begin{exampleblock}{Definición}
Supongamos que $S$ es un sistema de reescritura y $u, v, w$ son monomios de modo que existen reglas $uv \rightsquigarrow f_1$ y $vw \rightsquigarrow f_2$ en $S$.
...
\end{itemize}
Si cualquiera de estas condiciones se cumple, el sistema $S$ es \emph{confluente}.
\end{block}
...
El sistema $\left\{\sigma_1=(yx\rightsquigarrow xy), \sigma_2=(zx\rightsquigarrow xz), \sigma_3=(zy\rightsquigarrow yz)\right\}$ es confluente:
\[
...
\end{tikzpicture}
\]
entonces \[k\langle \Hat{Q}\rangle \simeq k[x]\]
\end{exampleblock}
\pause

\begin{exampleblock}{Ejemplo}
Si $\tilde{Q}$ es el quiver:
\[
...
\end{tikzpicture}
\]
entonces \[k\langle \tilde{Q}\rangle \simeq k\langle x_1,\dots,x_n\rangle\]
\end{exampleblock}
\end{frame}
...
\[\sum_{k=1}^n \lambda_k u_k \longrightarrow \sum_{k=1}^\infty \lambda_k u_k.\]
\end{frame}
\begin{frame}
\begin{exampleblock}{Definición}
...
\pause
\begin{exampleblock}{Ejemplo}
En $k\langle\!\langle \hat{Q}\rangle\!\rangle$, si notamos $x$ a la única flecha del quiver:
...
\bigskip
\begin{exampleblock}{Ejemplo}
Con la notación previa, si en $\hat{Q}$ consideramos $P=x^n$,
\[J(\hat{Q},P) = k[x]/(x^n) \]
\end{exampleblock}
\end{frame}
...
\begin{frame}
\begin{exampleblock}{Definición}
Una \emph{superficie} es una variedad diferencial sin borde, compacta, orientable y de dimensión 2.
\end{exampleblock}
\bigskip
\pause
\begin{block}{Teorema}
...
\item Para $g>0$, el toro con $g$ agujeros es la superficie de género $g$.
\end{itemize}
\end{block}
\end{frame}
...
\tikzstyle{every node} = [circle, fill=blue, minimum size=.05cm, inner sep=0cm]
\node (c) at (0,0) {};
\node (n) at (0,4) {};
\node (sw) at (210:4) {};
\node (se) at (330:4) {};
