...
For the cases in which $g>0$, we will make use of the handle decomposition we mentioned previously. We will draw a handle as a square (with opposite edges identified) having a gray hole in its center.
...
\end{tikzpicture}
\]
\caption{A decomposed handle.}
\label{divided-handle}
\end{figure}

We first deal with the case in which $g=1$. If we glue a handle decomposed as in \hyperref[divided-handle]{Figure \ref*{divided-handle}} to the boundary of the disk pictured in \hyperref[divided-handle]{Figure \ref*{divided-disk}}, we obtain a torus with a single red loop $r_1$ and a single blue loop $b_1$.
...
\end{tikzpicture}
\]
\caption{A decomposed disk with holes.}
\label{holed-up-disk}
\end{figure}
...
\end{tikzpicture}
\]
\caption{The quiver associated to a prism.}
\label{prism-quiver}
\end{figure}
